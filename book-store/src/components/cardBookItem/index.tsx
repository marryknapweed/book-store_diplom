...
//   const quantityInCart = cartItems.find(item => item.isbn13 === isbn13)?.quantity || 0 // Получаем количество товаров в корзине для текущего элемента
...
//               <QuantityControl quantity={quantityInCart} onIncrement={handleAddToCart} onDecrement={handleRemoveFromCart} />
//             </div>
  ...
  return (
    <>
      ...
      {activeTab === 'Similar Books' && (
        <div className="book-details__similar-books">
          <ul className="book-details__similar-books-list">
            {getRandomBooks(6).map((book: Book) => (
                <CardBook key={book.isbn13} {...book} />
            ))}
          </ul>
        </div>
